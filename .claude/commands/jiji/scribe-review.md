...
4. Commit the DD change in `dd_commit_repo` (staging only the DD file so the commit-msg hook's `EXEMPT_PATHS` exemption applies), trailer `AI-Assisted: scribe (<model>)`, and bump the status doc `private/docs/status.md` Resume cue.
