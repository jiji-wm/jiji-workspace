---
description: Append a Reviewed: entry to a jiji target's DD for one or more landed commits via jiji-scribe. First arg selects the target from the loop registry.
...
2. Resolve `dd_path` and `dd_commit_repo` from the registry, and read the most recent two to three `Reviewed:` blocks in the target's DD (or the sibling DD if this is its first block) to match voice.
