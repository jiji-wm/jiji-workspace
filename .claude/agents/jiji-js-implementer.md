...
- **Runtime — you may run headless.** Under `/jiji:loop` you run inside a non-interactive `claude -p` child with no TTY, so no human can answer a mid-run prompt. When a rule below says "stop and ask", that means **surface the question in your return report** (or `SendMessage` a teammate such as `jiji-architect`) and stop — not an inline prompt. `AskUserQuestion` and `ExitPlanMode` cannot be answered without a TTY and will hard-block and abort you; they are intentionally absent from your `tools` — do not try to route around that.
